"use client";

import { motion } from "framer-motion";
import Link from "next/link";
import {
  Star,
  ArrowRight,
  CheckCircle,
  Calendar,
  Users,
  Sparkles,
  BookOpen,
  MessageCircle,
  Award,
} from "lucide-react";

type Feature = {
  icon: React.ElementType;
  title: string;
  description: string;
};

export default function AstrojyotishPage() {
  const features: Feature[] = [
    {
      icon: Calendar,
      title: "Birth Chart Analysis",
      description:
        "Detailed Vedic astrology birth chart with planetary positions, house interpretations, and life predictions.",
    },
    {
      icon: Sparkles,
      title: "Daily Predictions",
      description:
        "Personalized daily horoscopes based on your unique astrological profile and planetary movements.",
    },
    {
      icon: Users,
      title: "Compatibility Reports",
      description:
        "Comprehensive compatibility analysis for relationships, partnerships, and marriage matching.",
    },
    {
      icon: MessageCircle,
      title: "Expert Consultations",
      description:
        "Connect with certified Vedic astrologers for personalized guidance and remedies.",
    },
    {
      icon: BookOpen,
      title: "Learning Resources",
      description:
        "Extensive library of Vedic astrology knowledge, tutorials, and interpretation guides.",
    },
    {
      icon: Award,
      title: "Gemstone Recommendations",
      description:
        "Personalized gemstone and remedy suggestions based on your birth chart analysis.",
    },
  ];

  const benefits: string[] = [
    "Accurate Vedic calculations",
    "Expert astrologer consultations",
    "Daily personalized insights",
    "Comprehensive life reports",
    "Relationship compatibility",
    "Remedial solutions and guidance",
  ];

  return (
    <div className="min-h-screen bg-white pt-20">
      {/* Hero Section */}
      <section className="relative py-20 overflow-hidden bg-gradient-to-br from-gray-50 to-white">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8 relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div
              initial={{ opacity: 0, x: -30 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6 }}
            > 

              <div className="flex items-center gap-3 mb-6">
                <div className="w-12 h-1 bg-[#E53935] rounded-full" />
                <span className="text-[#E53935] text-sm font-semibold tracking-wider uppercase">
                  Vedic Astrology Platform
                </span>
              </div>

              <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-6">
                Astrojyotish
              </h1>

              <p className="text-gray-600 mb-8">
                Comprehensive Vedic astrology and horoscope platform offering
                personalized insights, daily predictions, and expert
                consultations based on ancient wisdom and modern technology.
              </p>

              <div className="flex flex-col sm:flex-row gap-4 mb-12">
                <Link
                  href="/contact"
                  className="bg-[#E53935] text-white hover:bg-[#c62828] px-8 py-4 rounded-lg transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl"
                >
                  Get Your Chart
                </Link>
                <button className="border-2 border-gray-300 hover:border-[#E53935] bg-white text-gray-900 px-8 py-4 rounded-lg transition-all duration-300">
                  Explore Features
                </button>
              </div>

              <div className="grid grid-cols-3 gap-6">
                {[
                  { value: "100K+", label: "Users" },
                  { value: "500+", label: "Astrologers" },
                  { value: "50+", label: "Years Wisdom" },
                ].map((stat) => (
                  <div key={stat.label} className="relative">
                    <div className="absolute top-0 left-0 w-full h-1 bg-[#E53935] rounded-full" />
                    <div className="pt-4">
                      <div className="text-gray-900 font-semibold mb-1">
                        {stat.value}
                      </div>
                      <div className="text-sm text-gray-600">{stat.label}</div>
                    </div>
                  </div>
                ))}
              </div>
            </motion.div>

            <motion.div
              initial={{ opacity: 0, x: 30 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6, delay: 0.2 }}
              className="relative"
            >
              <div className="aspect-square bg-gradient-to-br from-[#E53935] to-[#c62828] rounded-3xl p-12 flex items-center justify-center relative shadow-2xl">
                <Star className="w-48 h-48 text-white" strokeWidth={1} />
              </div>
            </motion.div>
          </div>
        </div>
      </section>

      {/* Features Grid */}
      <section className="py-32 bg-white">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.6 }}
            className="text-center max-w-3xl mx-auto mb-16"
          >
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="w-12 h-1 bg-[#E53935] rounded-full" />
              <span className="text-[#E53935] text-sm font-semibold tracking-wider uppercase">
                Platform Features
              </span>
              <div className="w-12 h-1 bg-[#E53935] rounded-full" />
            </div>
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-6">
              Ancient Wisdom Meets Modern Technology
            </h2>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => {
              const Icon = feature.icon;
              return (
                <motion.div
                  key={feature.title}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1, duration: 0.4 }}
                  className="bg-white rounded-2xl border border-gray-200 p-8 hover:border-[#E53935] hover:shadow-xl transition-all duration-300 group"
                >
                  <div className="w-14 h-14 bg-[#E53935] rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                    <Icon className="w-7 h-7 text-white" strokeWidth={2} />
                  </div>

                  <h3 className="text-xl font-semibold text-gray-900 mb-3">
                    {feature.title}
                  </h3>
                  <p className="text-gray-600">{feature.description}</p>
                </motion.div>
              );
            })}
          </div>
        </div>
      </section>

      {/* Benefits Section */}
      <section className="py-32 bg-gray-50">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div
              initial={{ opacity: 0, x: -30 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 0.6 }}
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="w-12 h-1 bg-[#E53935] rounded-full" />
                <span className="text-[#E53935] text-sm font-semibold tracking-wider uppercase">
                  What You Get
                </span>
              </div>

              <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-6">
                Comprehensive Astrological Guidance
              </h2>

              <p className="text-gray-600 mb-8">
                Astrojyotish combines traditional Vedic astrology with modern
                technology to provide accurate predictions and guidance for every
                aspect of life.
              </p>

              <ul className="space-y-4">
                {benefits.map((benefit) => (
                  <li key={benefit} className="flex items-start gap-3">
                    <CheckCircle
                      className="w-6 h-6 text-[#E53935] flex-shrink-0 mt-0.5"
                      strokeWidth={2}
                    />
                    <span className="text-gray-600">{benefit}</span>
                  </li>
                ))}
              </ul>
            </motion.div>

            <motion.div
              initial={{ opacity: 0, x: 30 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 0.6 }}
              className="bg-gradient-to-br from-[#E53935] to-[#c62828] rounded-3xl p-12 text-white shadow-2xl"
            >
              <Sparkles className="w-16 h-16 mb-6" strokeWidth={1.5} />
              <h3 className="text-2xl font-semibold mb-4">
                Trusted by Thousands
              </h3>
              <p className="mb-6 text-white/90">
                Join over 100,000 users who trust Astrojyotish for accurate Vedic
                astrology insights and guidance from certified expert
                astrologers.
              </p>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <div className="text-2xl font-bold mb-1">4.9/5</div>
                  <div className="text-white/80 text-sm">User Rating</div>
                </div>
                <div>
                  <div className="text-2xl font-bold mb-1">1M+</div>
                  <div className="text-white/80 text-sm">Consultations</div>
                </div>
              </div>
            </motion.div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-32 bg-white">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-100px" }}
            transition={{ duration: 0.6 }}
            className="bg-gradient-to-br from-[#E53935] to-[#c62828] rounded-3xl p-12 lg:p-16 text-center text-white shadow-2xl"
          >
            <h2 className="text-3xl lg:text-4xl font-bold mb-6">
              Discover Your Cosmic Path
            </h2>
            <p className="mb-8 text-white/90 max-w-2xl mx-auto">
              Get personalized astrological insights and guidance. Start your
              journey today with a free birth chart.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                href="/contact"
                className="bg-white text-[#E53935] hover:bg-gray-50 px-8 py-4 rounded-lg transition-all duration-300 flex items-center justify-center min-w-[200px] shadow-lg hover:shadow-xl"
              >
                Get Started
              </Link>
              <Link
                href="/products"
                className="border-2 border-white hover:bg-white/10 text-white px-8 py-4 rounded-lg transition-all duration-300 flex items-center justify-center min-w-[200px]"
              >
                View All Products
              </Link>
            </div>
          </motion.div>
        </div>
      </section>
    </div>
  );
}
